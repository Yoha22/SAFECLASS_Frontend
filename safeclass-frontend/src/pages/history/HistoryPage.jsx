...
export default function HistoryPage() {
  const { alerts, confirmAlert, discardAlert, escalateAlert } = useAlerts();
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-3.5 border-b border-[#1e2d4a] flex items-center gap-4 shrink-0 flex-wrap">
        ...
        <div className="relative ml-auto">
          ...
          <input
            type="text" placeholder="Buscar ID o aula..."
          ...
          <thead className="sticky top-0 bg-[#0d1321] z-10">
            <tr>
              {['ID', 'Tipo', 'Aula', 'Hora', 'Confianza', 'Estado', 'Escalada'].map((h) => (
                <th key={h} className="px-4 py-2.5 text-left text-[11px] text-text-hint font-semibold tracking-wider border-b border-[#1e2d4a]">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
